...
app.post("/api/upload-image", async (req, res) => {
    ...
    if (!billId || !imageData) {
        return res
            .status(400)
            .json({ error: "Bill ID and image data are required" });
    }
    ...
    try {
        ...
        if (imageBuffer.length > MAX_IMAGE_SIZE) {
            return res
                .status(400)
                .json({ error: "Image size exceeds maximum allowed size of 10MB" });
        }

        // Remove any existing images for this billId with allowed extensions
        await Promise.all(
            ALLOWED_EXTENSIONS.map(async (ext) => {
...
app.post("/api/extract-bill", async (req, res) => {
    ...
    if (!isBase64Image(imageData)) {
        return res
            .status(400)
            .json({ error: "Invalid image data format. Expected base64 data URL." });
    }
...
app.delete("/api/delete-image/:billId", async (req, res) => {
    ...
    try {
        ...
        if (bill) {
            return res
                .status(403)
                .json({ error: "Cannot delete image for existing bill" });
        }
...
app.get("/api/stats", async (req, res) => {
    try {
        ...
        // Calculate total
        const query = `SELECT COALESCE(SUM(total), 0) as total FROM bills ${whereClause}`;
